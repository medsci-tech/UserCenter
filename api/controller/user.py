...
def _editUser(param):
    ...
    try:
        ...
        return {'contract_code': 808, 'msg': '数据验证错误', 'data': None}
    if model:
        returnData = {'contract_code': 200, 'msg': '操作成功', 'data': str(model['id'])}
    else:
        returnData = {'contract_code': 800, 'msg': '操作失败', 'data': None}
    return returnData
...
def edit(request):
    post = request.POST
    if not post:
        returnData = {'contract_code': 900, 'msg': '非法请求', 'data': None}
        return HttpResponse(json.dumps(returnData), content_type="application/json")
    uc_uid = post.get('uc_uid')
    token = post.get('token')
    check_token = QXToken(uc_uid).verify_auth_token(token)
    if check_token == None:
        return {'contract_code': 901, 'msg': '非法操作', 'data': None}
    param = {
    ...
    if result.get('contract_code') == 200:
        returnData = {'contract_code': 200, 'msg': '成功', 'data': None}
    else:
        returnData = result
